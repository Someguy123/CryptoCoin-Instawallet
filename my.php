...
<div class="row push">
  <?php
    echo srsnot ( "<strong>IMPORTANT!</strong><br /><strong>Don't lose this link!</strong> This link contains your <strong>Wallet Identifier</strong>&mdash;if you lose it, you won't be able to find your account.<br />
    <center><a href=\"/key/{$_GET['key']}\" style=\"font-size: 12px;\">http://{$_SERVER['SERVER_NAME']}/key/{$_GET['key']}</a> (ctrl+D to bookmark)</center>" );
  ?>
  <div class="span10">
    <div class="page-header">
      <h4>Your <?php echo $coin; ?> address:</h4>
          ...
          if($_POST['newpass'] == $_POST['passconf']) {
            try {
              if($addr->hasPass($_GET['key'])) {
                ...
                echo srsnot("Your password has been set successfully; we recommend you log out and back in again to finish setting the password correctly.");
              } else {
              ...
              switch($error) {
                case "INV_PASSWORD":
                  echo srserr("You typed your current password incorrectly, please go back and enter the correct password.");
                  break;
                case "TOO_LONG":
            ...
            echo srserr("Please make sure you typed your password correctly.");
          }
        }
        $ltcaddr = $addr->verKey ( $_GET ['key'] );
